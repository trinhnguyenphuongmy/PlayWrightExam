...
test.describe("Verify Homepage with three Sliders only", () => {
  test("Verify Homepage with three Sliders only", async ({ pages }) => {
    ...
    logger.info("Test whether the Home page has Three Sliders only");
    logger.info("The Home page must contains only three sliders");
    // 5) Test whether the Home page has Three Sliders only
    // 6) The Home page must contains only three sliders
    await test.step("5, 6) Test whether the Home page has Three Sliders only", async () => {
      expect(await homePage.countNumberOfSliders()).toBe(3);
    });
  });
});
